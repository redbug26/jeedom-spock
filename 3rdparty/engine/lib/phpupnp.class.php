...
class phpUPnP {
	...
	public function mSearch($st = 'ssdp:all', $deviceIp = '239.255.255.250', $mx = 1, $man = 'ssdp:discover', $from = null, $port = null, $sockTimout = '2') {
		...
		do {
			unset($buf);
			@socket_recvfrom($sock, $buf, 1024, MSG_WAITALL, $remote_ip, $remote_port);

			//	@socket_recv($sock, $buf, 1024, MSG_WAITALL); //, $from, $port );
			if (!is_null($buf)) {
				$parsedResponse = $this->parseMSearchResponse($buf);
				$parsedResponse["ip"] = $remote_ip;
	...
	private function parseMSearchResponse($response) {
		...
		foreach ($responseArr as $row) {
			if (stripos($row, 'http') === 0) {
				$parsedResponse['http'] = trim($row);
			} else if (stripos($row, 'cach') === 0) {
				$parsedResponse['cache-control'] = trim(str_ireplace('cache-control:', '', $row));
			} else if (stripos($row, 'date') === 0) {
				$parsedResponse['date'] = trim(str_ireplace('date:', '', $row));
			} else if (stripos($row, 'ext') === 0) {
				$parsedResponse['ext'] = trim(str_ireplace('ext:', '', $row));
			} else if (stripos($row, 'loca') === 0) {
				$parsedResponse['location'] = trim(str_ireplace('location:', '', $row));
			} else if (stripos($row, 'serv') === 0) {
				$parsedResponse['server'] = trim(str_ireplace('server:', '', $row));
			} else if (stripos($row, 'st:') === 0) {
				$parsedResponse['st'] = trim(str_ireplace('st:', '', $row));
			} else if (stripos($row, 'usn:') === 0) {
				$parsedResponse['usn'] = trim(str_ireplace('usn:', '', $row));
			} else if (stripos($row, 'cont') === 0) {
				$parsedResponse['content-length'] = trim(str_ireplace('content-length:', '', $row));
			} else {
				if (trim($row) != "") {
					$parsedResponse[] = $row;
				}
